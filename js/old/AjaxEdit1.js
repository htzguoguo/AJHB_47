...
//    $("[id*='edit']").live('click', OnEdit);
    ...
    $(document).on("click", "[id*='edita']", OnEdit1);
    $("[id*='add']").click(OnAdd);
...
//    ration.before("<input id='span6Edit' type='text' value='" + ration.text() + "'/>").hide();
//    mamount.before("<input id='span7Edit' type='text' value='" + mamount.text() + "'/>").hide();
...
//    c1.before("<input id='span11Edit' type='text' value='" + c1.text() + "'/>").hide();
...
//    c3.before("<input id='span13Edit' type='text' value='" + c3.text() + "'/>").hide();
   ...
   // tr.find("[id*='delete']").hide();
    tr.find("[id*='edita']").before("<img id='save1' src='images/b4.jpg' />")
        .hide();

    tr.find("[id*='save1']").one('click', OnSave1);
}
...
//    c3.remove();
  ...
  // tr.find("[id*='delete']").show();
    tr.find("[id*='edita']").show();
    tr.find("[id*='save1']").remove();

    // update the contact on the server
    UpdateContact1(tr.attr("id"), amount.val(), memo.val(), price.val());
}
...
function UpdateContact1(id, amount, memo,price) {
    var data = '{'
            + "\"id\":" + id + ","
...
//                             + "\"ca\":\"" + c1 + "\","
    ...
    $.ajax({
        type: "POST",
        url: "MonthlyYangHuJiHua2.aspx/UpdateContact",
        data: data,
...
//                                           + "\"hesuan\":\"" + aspxtxthesuan.GetText() + "\""
    ...
    $.ajax({
        type: "POST",
        url: "MonthlyYangHuJiHua2.aspx/AddContact",
        data: data,
        contentType: "application/json",
        dataType: "json",
        error: OnAjaxError,
